...
3. **Choose** `pax` or `cargo` option (`pax` by default)
4. **Click** the `📁` button
5. **Choose** your JSON file (generated by `am4help` bot)
6. **Done!** Just copy the generated file path. The Excel file (`.xlsx`) will be created in the same folder as the (`.json`) file
